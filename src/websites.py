...
class Websites:
    try:
        ...
        def bag_snob(self):
            ...
            web_data = BeautifulSoup(requests.get(bag_snob + '{}'.format(tag)).text, 'html.parser').find_all('div', {'class': 'col-md-8 content-area'})
            feeling_lucky = random.choice(web_data)
            link = feeling_lucky.find('a')['href']
            text = '#bagsnob #{} '.format(tag) + feeling_lucky.find('h1').text.strip()
            return text[:100], link
